...
stError gpio_init(void)
{
    ...
    sprintf(gpiochip_file, "/dev/gpiochip0"); /* Open /dev/gpiochip0 */
...
#if defined(ST25R95)
    ST25R95_N_IRQ_OUT_PORT  = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_INPUT, ST25R95_N_IRQ_OUT_PIN);
#else
    ST25R_INT_PORT          = gpio_get_lineevent(fd, ST25R_INT_PIN);
#endif

#if defined(ST25R_RESET_PIN)
    ST25R_RESET_PORT        = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, ST25R_RESET_PIN);
#endif
#if defined(PLATFORM_LED_FIELD_PIN)
    PLATFORM_LED_FIELD_PORT = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_FIELD_PIN);
#endif
#if defined(PLATFORM_LED_A_PIN)
    PLATFORM_LED_A_PORT     = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_A_PIN);
#endif
#if defined(PLATFORM_LED_B_PIN)
    PLATFORM_LED_B_PORT     = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_B_PIN);
#endif
#if defined(PLATFORM_LED_F_PIN)
    PLATFORM_LED_F_PORT     = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_F_PIN);
#endif
#if defined(PLATFORM_LED_V_PIN)
    PLATFORM_LED_V_PORT     = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_V_PIN);
#endif
#if defined(PLATFORM_LED_AP2P_PIN)
    PLATFORM_LED_AP2P_PORT  = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_AP2P_PIN);
#endif
#if defined(PLATFORM_LED_RX_PIN)
    PLATFORM_LED_RX_PORT  = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, PLATFORM_LED_RX_PIN);
#endif

#if defined(ST25R95)
    ST25R95_SSI0_PORT       = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, ST25R95_SSI0_PIN);
    ST25R95_N_SS_PORT       = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, ST25R95_N_SS_PIN);
    ST25R95_N_IRQ_IN_PORT   = gpio_get_linehandle(fd, GPIOHANDLE_REQUEST_OUTPUT, ST25R95_N_IRQ_IN_PIN);
#endif
    ...
    /* Set GPIOs initialized allows the following settings without recursive calls to gpio_init() */
    isGPIOInit = true;

#if defined(ST25R95)
    /* Set SSI 0 */
    gpio_set(ST25R95_SSI0_PORT, ST25R95_SSI0_PIN);
    platformDelay(1);
    /* Clear IRQ In */
    gpio_clear(ST25R95_N_IRQ_IN_PORT, ST25R95_N_IRQ_IN_PIN);
    /* Set N SS */
    gpio_set(ST25R95_N_SS_PORT, ST25R95_N_SS_PIN);
#endif
...
void* pthread_func()
{
    ...
    while (true)
    {
        ...
        /* The port is used to contain the file descriptor of a given pin! */
#if defined(ST25R95)
        ssize_t s = read(ST25R95_N_IRQ_OUT_PORT, &event, sizeof(event));
#else
...
void gpio_set_value(int port, int pin_no, uint8_t value)
{
    ...
    if (ret < 0) {
        fprintf(stderr, "Failed to set GPIO pin%d value %d (errno %d)\n", pin_no, data.values[0], errno);
    }
}
